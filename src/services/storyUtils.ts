...
import { IllustrationCreationParams, IllustrationCreationParamsData, StoryMediaCreationOutput, StoryMediaCreationOutputData, SentenceRichness, SentenceRichnessData, SimilarityCheck, SimilarityCheckData, StoryTranslation, StoryTranslationData } from '../data'
...
import { gifProcessing, imageProcessing } from '.'
...
const utils = {
     ...
     * - 2 - LONGEST SENTENCES
     */
    sortSimilarSentencesByRichest(sentences_richness: SentenceRichnessData[]) {
     ...
     * - 2 - NOUNS COUNT
    */
    sortUnsimilarSentencesByRichness(sentences_richness: SentenceRichnessData[]) {
    ...
    trimUsedKeywords(similarities: SimilarityCheckData[], used_keywords: string[]) {
    ...
    trimUsedKeywordsFromNouns(nouns: string[], used_keywords: string[]) {
        return nouns
    ...
    /** 
     * Lacks of precision resulting in less paragraphs than wanted. 
     * e.g. 8 sentences into 5/6/7 paragraphs will output 4 paragraphs
    */
    splitEquallyIntoParagraphs(text: string, desiredBatchCount: number) {
    ...
    /** See docs/min_reading_time_calculation.md */
    estimateMinReadingTime(text_characters: number) {
        ...
        // - so it works if people are read quickly 
        // - catches the lack of precision from the `average_characters_per_second`
        // Anyway this value must not be very precise, it is just designed so that people don't win 
        // a story's collectible in 10 sec (but in 1-2 or 5 min for instance).
    },
}
...
/**  
 * - Role: Extracts the key passages of the story in order to split it's text 
 * into paragraphs and in order to have the best illustrations of these passages. 
 * 
 * - Steps : 
 *   - 1 - calculate the avg lenght of a paragraph based on story's text lenght 
 *         and split the text into equaly long paragraphs
 *   - 2 - get the most pertinent sentence of each paragraph
*/
export function analyseAndExtractRichSentencesForIllustrationsV2(text: string, keywords: string[], desired_paragraph_count: number) {
    ...
    const paragraphs_sentences = utils.splitEquallyIntoParagraphs(text, desired_paragraph_count)
    ...
    var illustrations_params: IllustrationCreationParamsData[] =
        Object.keys(sentences_richness).flatMap((p_index) => {
            ...
            var best_s_with_similarities = utils.sortSimilarSentencesByRichest(paragraph_sentences)
            var best_s_without_similarities = utils.sortUnsimilarSentencesByRichness(paragraph_sentences)
...
export async function createStoryTranslation(
    ...
    return new Promise<StoryTranslationData>(async (resolve) => {
        ...
        // OUTPUT
        const paragraphs_sentences = utils.
