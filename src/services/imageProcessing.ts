...
export function hexToRgba(hex: string) {
    ...
    // uses an alpha value of 255 when the hex does not contain an alpha
    // value, e.g. "#4bf542"
 ...
 * Throws an error if no color is provided.
 * 
...
async function getEqualizedImage(originalImg: Jimp, tolerance: number, whiteColor: RGBAcolor, lineColor: RGBAcolor) {
    const equalizedImage = originalImg.clone().normalize(); // Create a clone of the image and normalize its color values

    // Compute the histogram for non-background pixels
    const histogram = new Array(256).fill(0); // Array to store the frequency of pixel intensities
    ...
    equalizedImage.scan(0, 0, equalizedImage.bitmap.width, equalizedImage.bitmap.height, function (x, y, idx) {
        ...
        if (!isBackgroundPixel) {
            ...
            histogram[intensity]++; // Increment the corresponding bin in the histogram
        }
    });

    // Compute the cumulative distribution function (CDF) for non-background pixels
    const cdf = new Array(256).fill(0); // Array to store the cumulative sum of histogram values
 ...
 * 
 */
export async function changeImageColor(imageBuffer: Buffer,
    ...
    const tolerance = useOptionalNumber(options?.tolerance, 34),
        ...
        lineColor = useColorParam(color),   // Can throw an ERROR
        CONTRAST = (1 - contrast),
        whiteColor = { r: 255, g: 255, b: 255, a: 255 };    // White color


    var image: Jimp = null as any  // -> init for ts only 
    try {
...
export async function changeAllPixelsColor(imageBuffer: Buffer,
    color: { hex?: string | null, rgba?: RGBAcolor | null }, reducePixelation = false) {

    const lineColor = useColorParam(color)   // Can throw an ERROR
