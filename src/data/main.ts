...
import { decrypt, serverSideEncryption } from "./../utils/functions/encryption"
import { getTranslation, setItem } from "./../backend/firebase/firestore"
import { createStoryTranslation } from "./../services"
...
// When an item is marked with the note : `// BACKEND` it indicates that 
// this DATA: 
// - is ONLY present on the SERVER.
// - is SENSITIVE data because it reveals a STEP of the algorithm and how
//   it WORKS globally.
// - must NOT BE SHARED to the CLIENT.
// - parts of this DATA can be SHARED to the CLIENT if it is properly 
//   formatted/simplified/obscurified into DATA designed for the CLIENT, 
//   in other words DATA marked with `// CLIENT FRIENDLY`. The quantity of
//   values that can be SHARED varies: sometimes NO value can be SHARED to 
//   the CLIENT and sometimes ONLY ONE CAN'T.

// When an item is marked with the note: `// CLIENT FRIENDLY` it indicates 
// that this DATA: 
// - is SUITABLE/COMPATIBLE for both the SERVER & the CLIENT.
// - In other words, this DATA can be BOTH present on the SERVER & the CLIENT
...
// BACKEND
export interface MatchingEmojiData {
    emoji: string
    best_matching_tag_index: number
    tags: string[]
    exact_match: boolean
}
export const MatchingEmoji = {
    object(emoji: string, best_matching_tag_index: number, tags: string[], exact_match: boolean) {
        return {
            emoji: emoji ?? null,
            best_matching_tag_index: best_matching_tag_index ?? null,
            tags: tags ?? null,
            exact_match: exact_match ?? null,
        }
    }
}
...
// BACKEND 
// (BUT IS CLIENT FRIENDLY. In other words try keeping this 
// data on the server but if you have to, you can use it on
// the client)
...
// CLIENT FRIENDLY data FORMATING
...
export const Keyword = {
    ...
    keys() {
        // Hacky way to get the keys an easy way.
...
export const StoryMedia = {
    ...
    // BACKEND
    /** 
     * - Formats the `"BACKEND"` data into a format that is `"CLIENT FRIENDLY"`. 
...
// FIREBASE USER PROFILE DATA EXTENSION HACK 
...
export const UserExtraInfo = {
    ...
    async fromUser(user: UserRecord | DecodedIdToken | null | undefined) {
        ...
        // REPLACE DEFAULT VALUES WITH CURRENT IF ANY 
        try {
...
// SECTION - Comments on data 
// Is it a good purpose to save on a dabatase all ids of the content the user 
// interacted with to then use it to recommend content by avoiding re-recommending
// the same content ?
//
// ANCHOR[id=collaborative_filtering] 
// Saving the IDs of the content the user has interacted with is a good approach 
// for implementing content recommendation systems. By keeping track of the user's
// past interactions, you can avoid recommending the same content repeatedly, which
// can improve the overall quality of the recommendations.
//
// This approach is commonly used in recommendation systems and is known as 
// "collaborative filtering." Collaborative filtering algorithms use the historical 
// behavior of users to find patterns and similarities between users, and use those 
// patterns to recommend items that the user has not yet interacted with.
// By storing the IDs of the content the user has interacted with, you can easily 
// filter out those items from the recommended list, and suggest new and relevant 
// items to the user.
// It's important to note that storing a large amount of user data can have privacy implications, and it's important to implement appropriate data protection measures, such as encryption and access controls, to protect the user's data. Additionally, it's important to obtain the user's consent before collecting and using their data for recommendation purposes.
// !SECTION